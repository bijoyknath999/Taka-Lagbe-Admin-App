...
public class PaymentActivity extends AppCompatActivity {
	...
	private String user_id = "", adminTotalClick = "";
			...
			if (_data.get((int)_position).containsKey("key")) {
				if (_data.get((int)_position).get("subject").toString().contains("withdraw")) {
					...
					if (_data.get((int)_position).containsKey("uid")) {
						user_id = _data.get((int)_position).get("uid").toString();
					}
										...
										if (!user_id.isEmpty() && user_id!=null)
										{
											users.child(user_id).addListenerForSingleValueEvent(new ValueEventListener() {
												...
												public void onDataChange(@NonNull DataSnapshot snapshot) {
													totalAmount = Double.parseDouble(_data.get((int)_position).get("amount").toString());
													adminTotalClick = snapshot.child("adminTotalClick").getValue().toString();
													m = new HashMap<>();
													m.put("adminTotalClick", String.valueOf((long)(Double.parseDouble(adminTotalClick)-totalAmount)));
													m.put("alRevenue", "0");
													m.put("alReward", "0");
													m.put("alInterstitial", "0");
													m.put("starReward", "0");
													m.put("starInterstitial", "0");
													users.child(user_id).updateChildren(m);
													m.clear();
